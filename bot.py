...
Добавлен минимальный HTTP-сервер для совместимости с Render Web Service.
"""
...
# ==================== HTTP-сервер для Render (фоновая задача) ====================
async def run_web_server():
    """Запускает минимальный HTTP-сервер, слушающий порт из переменной окружения PORT."""
    app = web.Application()

    async def handle(request):
        return web.Response(text="Bot is running")

    # Обрабатываем любые GET и POST запросы (Render может проверять корневой путь)
    ...
    # Бесконечно ждём, но не блокируем основной цикл (запущено как фоновая задача)
...
# ==================== Обработчик стикеров (для поддержки премиум стикеров) ====================
@dp.message(F.sticker)
async def handle_sticker(message: Message):
    # Пересылаем стикер обратно
